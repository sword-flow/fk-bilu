...
Redis通过**复制偏移量、复制积压缓冲区、服务器运行ID**三个部分实现。首先从服务器发送复制命令给主服务器，开始同步，如果中断，也就是判断出不是第一次复制，则向服务器发送同步命令，并且带上服务运行ID和复制偏移量，主服务器根据便宜量从复制积压缓冲区里将数据发送到从服务器中。

#### 哨兵

哨兵服务器只是一个特殊的Redis服务器，所以可以使用的命令不太一样。
